...
| `guard-shadow` (breadcrumb, **`resources/bram-traces/hook-events.log`**, not `bram-trace.log`) | `bram guard <hook>` reentrant mode (`src-tauri/src/guard.rs`), lines tagged `claude-rs` / `codex-rs` in the provider column | `<ts> <claude-rs\|codex-rs> <event> <tool> would-post=<route>\|action=none port=<n\|none> ms=<n>` | bram-guard-reentrant-menu-hooks phase 1 (the Python-hook retirement, receipts on #269): the Rust port of the permission-menu hooks runs in SHADOW — never POSTs, never decides, always exits 0 — and appends one breadcrumb per invocation into the same `hook-events.log` the Python menu hooks write, so the divergence check is a grep-join: every Python breadcrumb should have an adjacent `-rs` twin whose `would-post` route matches the Python line's `post=` route, and an `-rs` line with no Python neighbor (or vice versa) is a divergence. `ms` is in-process handling time; spawn cost is measured separately in the soak. Registered for Claude by Setup beside the Python menu hook (marker `bram-guard`, outside the needs-setup calculus); Codex registration waits for the worklist-policy item so the config.toml re-trust prompt fires once. |
