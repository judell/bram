...
| `pty-menu` `op=boot-prompt` (host) | allowlisted boot-prompt classifier at the top of `pty_menu_update`, `lib.rs` (`codex_trust_prompt_menu_from_chunk`) | `shape=codex-hooks-trust` | codex-trust-prompt-first-class-menu: the Codex hooks re-trust prompt surfaces as a real pane menu (options answer via `sendKeys` digit + CR, which also gives the answer an audit line terminal keystrokes lack). Classification is per-chunk BY DESIGN — the prompt paints once via cursor addressing, and the #178 turn-start drain can wipe the rolling tail before any later evaluation reads it (the race that left terminal-attention's classified `hooks-trust` shape firing `op=candidate preview=""` with no banner, 2026-08-28 22:32Z; the attention reader now falls back to a drained-tail stash for the banner floor). Bypasses `menus.parseAndDisplay` for this classified shape only; guards: Codex provider, no open working turn (an agent echoing the prompt text must not conjure a phantom menu), no menu already displayed. Clear rides the standard dismissal lifecycle (user input, pane answer, session rotation) — the prompt has no teardown byte, only a full session repaint. |
