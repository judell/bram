...
| `w2-selection` | selection `ChangeListener`s in `Worklist.xmlui`; the `gateSel` `ChangeListener` in `WorklistGateBar.xmlui` | `op=prune-input` (`items`, `selected`); `op=publish` (`count`, `items`, `store`); `op=receive` (`count`, `render`, `store`) | instrument-w2-selection-handoff: the Worklist -> footer selection handoff, made self-reporting. Selection crosses a window store (`__bramW2SetSelection` / `bramSubscribeW2Selection`) that is written **only on change** - the setter early-returns on an equal array - so nothing ever republishes the current value. Observed 2026-08-24: the footer gate bar rendered **no buttons** while the row's checkbox was still ticked. Absent, not greyed, is the discriminator - the buttons gate on `enabled=` but their container gates on `when="{(gateSel.value || []).length > 0}"`, so the footer's store was empty while the page's `worklist2Selected` was not. Leading hypothesis these fields exist to test: the prune `ChangeListener` (`Worklist.xmlui`, on `worklist.value.items`) fires during a refetch that momentarily carries zero items, wiping the selection, while the row `Checkbox` uses `initialValue` - **uncontrolled**, the same class as the `ExpandableItem` trap - and keeps showing ticked. That reads `op=prune-input items=0` followed by `op=publish count=0 items=0` with no `op=receive count>0`. Ruled out before instrumenting, none of which the trace could have told us: late-subscriber cold start (the factory calls `fire()` on subscribe), a stuck claim (`op=clear` fired normally), and a pane reload (absent from the trace). **A mismatch tripwire, not a zero-is-success one**: steady state emits publish/receive PAIRS, so the anomaly is a publish with no matching receive, or a `receive count=0` beside a live selection - not the absence of lines. Retire it once a recurrence names the mechanism and the fix lands; if it never recurs in normal use, say so and remove it rather than leaving a permanent instrument on a one-off. |
