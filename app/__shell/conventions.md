...
indexer's `issues-changed` signal (no polling), and its ↻ Refresh button
forces a live forge fetch that also rewrites the cached list, so updates
surface without a restart. The forge is detected from the
