...
| `inflight-collision` | `write_inflight_claim_sentinel` / `clear_inflight_claim_sentinel` / `consume_worklist_authorization` in `lib.rs` | `op=write` (+`prior_ids`, `prior_kind`, `prior_age_ms` when displacing a live claim); `op=clear-partial` (`claimed`, `requested`, `remaining`); `[auth-record] op=consume-already-consumed` (`kind`, `consumed_at_ms`, `ids`) | subagent-worklist-collision-observability: observe-only detection of the three ways concurrent subagent delegation collides on single-claimant host state — a claim overwritten while live, a clear covering only part of the active claim, and a second consumer finding the authorization record already spent. Steady-state single-agent sessions emit none of these; any occurrence is a collision. |
