...
| `send-ledger` (host) | ledger transitions and guards, `lib.rs` | `op=inject/transition/restore/auto-resend/aborted-no-restore/aborted-skip/stale-input-clear/stale-input-clear-skip` with entry ids, causes, byte counts; `op=would-suppress-strand id=<id> reason=sid-changed old=<basename> new=<basename>` | Outbound-send lifecycle forensics: landing vs strand vs abort classification, restores, and the stale-terminal-input clear decisions. `would-suppress-strand` is the observe-only phase of send-ledger-observe-sid-switch-strand: emitted when a mechanical strand's active session file differs from the one the send was injected against (a rollover false-strand), with no behavior change yet — graduation to actually suppressing the strand needs the soak to confirm every fire is a real rollover where the send landed. |
