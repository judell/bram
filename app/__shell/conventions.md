...
| `search-index` (host) | `start_search_indexer` / `run_search_index_pass` and the `__search` route in `lib.rs` | `op=scan bucket=<b> files=<n> indexed=<n> skipped=<n> rows=<n> ms=<n>` per incremental pass; `op=scan-error bucket=<b> detail=<…>`; `op=query chars=<n> hits=<n> ms=<n>` per `/__search` request | issue-230 unified full-text search (embedded SQLite FTS5, `search_index.rs`). The background indexer keeps the current project's session transcripts in an FTS5 index (`bucket=claude` first; Codex next) stored as a rebuildable cache at `<app_cache_dir>/search-index/<project-key>.db`; `op=scan` quantifies each incremental pass (skipped = unchanged files, no re-read) and `op=query` the read-path latency — together they replace the full-re-read-per-query cost of `search_sessions()` and let us compare against that baseline. |
